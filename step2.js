...
var CtrlStep2 = function () {
    return {
		...
		loadKomponenNewForm: function () {
			...
			$("#btnCariRujukan1").click(function(){
				recordrujukanx = undefined;
				var nikPasien = $("#nikPsn").val(),
					noKartu = $("#nokartuPsn").val();
				ONextLoader.onDialogProses('Loading ...');
				...
				$.ajax({
					...
					success: function(response){
						if(response.success){
							...
							for(var i= 0; i<jmlData; i++){
								var tabelData = '<option value="'+response.data[i].ID+'" datadokter="'+response.data[i].KD_DOKTER+'" datanamadokter="'+response.data[i].NM_DOKTER+'" datajam="'+response.data[i].JAM+'" datapenjamindokter="'+response.data[i].KD_SUB_SPESIALIS+'">'+response.data[i].NM_DOKTER+' | Pukul : '+response.data[i].JAM+' ( '+response.data[i].KD_SUB_SPESIALIS+' )</option>';
			...
			$("#btnVerifikasi").on('click',(function(e) {
				var jenisPasien = $("#jenisPasien").val(),
					...
					kdPoliBpjs = $("#jadwalDokterTujuan")[0].options[xb].getAttribute('datapenjamindokter'),
